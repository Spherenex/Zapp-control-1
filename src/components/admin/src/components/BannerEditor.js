...
// // Helper function to check if a banner should be active based on scheduling
// const isBannerActiveBasedOnSchedule = (banner) => {
//   if (!banner) return false;
  
//   // If no scheduling, use the banner's isActive flag
//   if (!banner.startDate && !banner.endDate) return banner.isActive;
...
//   // Check and update banner active status based on scheduling
//   useEffect(() => {
//     if (Object.keys(banners).length === 0) return;
    
//     // First check on component mount
//     updateBannerActiveStatus();
    
//     // Set up interval to check every minute
//     const intervalId = setInterval(updateBannerActiveStatus, 60000);
    
//     return () => clearInterval(intervalId);
//   }, [banners]);
  
//   // Function to update banner active status based on scheduling
//   const updateBannerActiveStatus = async () => {
//     const updates = {};
//     let needsUpdate = false;
    
//     Object.entries(banners).forEach(([key, banner]) => {
//       // Skip banners without scheduling
//       if (!banner.startDate && !banner.endDate) return;
      
//       const shouldBeActive = isBannerActiveBasedOnSchedule(banner);
      
//       // If there's a difference between current status and what it should be
//       if (banner.isActive !== shouldBeActive) {
//         updates[`${key}/isActive`] = shouldBeActive;
//         needsUpdate = true;
//       }
//     });
    
//     // If any banners need updating, update the database
//     if (needsUpdate) {
//       try {
//         const bannersRef = dbRef(db, 'banners');
//         await update(bannersRef, updates);
        
//         // Update local state
//         setBanners(prev => {
//           const updated = { ...prev };
//           Object.entries(updates).forEach(([path, value]) => {
//             const [bannerKey] = path.split('/');
//             if (updated[bannerKey]) {
//               updated[bannerKey] = {
//                 ...updated[bannerKey],
//                 isActive: value
//               };
...
//         console.log("Updated banner statuses based on scheduling");
//       } catch (error) {
...
//     if (!formData.title.trim() || !formData.productName.trim()) {
...
//       // Check if banner should be active based on scheduling
//       if (schedulingEnabled) {
//         dataToSave.isActive = isBannerActiveBasedOnSchedule(dataToSave);
//       }
...
//   // Get the actual active status for a banner (considering scheduling)
//   const getActualBannerStatus = (banner) => {
//     if (!banner) return false;
//     return isBannerActiveBasedOnSchedule(banner);
//   };
...
//       key,
//       ...data,
//       actualActive: getActualBannerStatus(data)
//     }));
//     return bannersArray.sort((a, b) => (a.order || 0) - (b.order || 0));
//   };

//   // Check if a banner is currently in scheduled mode
...
//                 <span className={`status-indicator ${banner.actualActive ? 'active' : 'inactive'}`}>
//                   {banner.actualActive ? 'Active' : 'Inactive'}
//                 </span>
//                 {isScheduled(banner) && (
//                   <span className="scheduled-badge" title="This banner has scheduling enabled">
//                     Scheduled
//                   </span>
//                 )}
//               </div>
...
//                   <div className="form-section banner-settings">
...
//                         {schedulingEnabled && (
//                           <span className="scheduling-note">
//                             Note: Scheduling will override this setting when start/end dates are reached
//                           </span>
//                         )}
//                       </div>
...
const BannerEditor = () => {
  ...
  // Filter state for active/inactive banners
  ...
  // Get sorted and filtered banners for display
  const getSortedBanners = () => {
    const bannersArray = Object.entries(banners).map(([key, data]) => ({
      key,
      ...data,
    ...
    // Apply filter
    const filteredBanners = bannersArray.filter(banner => {
      if (activeFilter === 'all') return true;
      return activeFilter === 'active' ? banner.actualActive : !banner.actualActive;
    });
    
    return filteredBanners.sort((a, b) => (a.order || 0) - (b.order || 0));
  };
